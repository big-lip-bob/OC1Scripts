...
local function sleep_check(t)
	return pull(min(t or 0,getEnergy()/energy_consumption_per_sec))
end
...
local function bind()
	while true do
		...
		if ev == "modem_message" and bind_sign == "__BIND" then
			return x,y,z,from
		else
			checkEnergy()
		end
...
local function get_target(master)
	while true do
		local ev,self_addr,from,port,distance,target_sign,x,z = sleep_check();checkEnergy()
		if ev == "modem_message" and from == master then
			if target_sign == "__GOTO" then return x,z
			else modem.broadcast(4,"__BOMBER_READY")
			end
		else
			checkEnergy()
		end
...
while true do
	if count(tntslot) < tntpillarlenght or count(buttonslot) < 1 or count(fuelslot) < 64 then exit() end
